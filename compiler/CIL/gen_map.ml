  ...
  let elementsP t0 kv m =
    ssr_have __
      (let _evar_0_ = fun _ _ -> ReflectT in
       ...
       (match boolP
                (in_mem (Obj.magic kv)
                  (mem (seq_predType (prod_eqType K.t t0))
 ...
 struct
  ...
  module Raw =
   struct
    ...
    | Leaf -> O
    | Node (_, l, _, r) -> S (Nat0.max (maxdepth l) (maxdepth r))

    (** val mindepth : tree -> nat **)

    let rec mindepth = function
    | Leaf -> O
    | Node (_, l, _, r) -> S (Nat0.min (mindepth l) (mindepth r))
    ...
    | R_concat_1 of tree * tree * Int.Z_as_Int.t * tree * Equality.sort * tree
