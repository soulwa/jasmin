...
module ByteSet =
 struct
  ...
  let rec _subset t1 t2 =
    ...
    | a :: l ->
      (match t2 with
       ...
       | a0 :: l0 ->
         if I.subset a a0
         then _subset l (a0 :: l0)
         else if Z.ltb a0.imax a.imin then _subset (a :: l) l0 else false)
  ...
  (** val _disjoint : bytes -> bytes -> bool **)

  let rec _disjoint t1 t2 =
    match t1 with
    | [] -> true
    | a :: l ->
      (match t2 with
       | [] -> true
       | a0 :: l0 ->
         if Z.leb a.imax a0.imin
         then _disjoint l (a0 :: l0)
         else if Z.leb a0.imax a.imin then _disjoint (a :: l) l0 else false)

  type _disjoint_ind =
  | I_disjoint_1 of bytes
  | I_disjoint_2 of bytes
  | I_disjoint_3 of interval * bytes * interval * interval list * bool
     * _disjoint_ind
  | I_disjoint_4 of interval * interval list * interval * bytes * bool
     * _disjoint_ind
  | I_disjoint_5 of interval * interval list * interval * interval list

  (** val _disjoint_ind_rect :
      (bytes -> 'a1) -> (bytes -> 'a1) -> (interval -> bytes -> interval ->
      interval list -> bool -> __ -> _disjoint_ind -> 'a1 -> 'a1) ->
      (interval -> interval list -> interval -> bytes -> bool -> __ ->
      _disjoint_ind -> 'a1 -> 'a1) -> (interval -> interval list -> interval
      -> interval list -> __ -> __ -> 'a1) -> bytes -> bytes -> bool ->
      _disjoint_ind -> 'a1 **)

  let rec _disjoint_ind_rect f f0 f1 f2 f3 _ _ _ = function
  | I_disjoint_1 t2 -> f t2
  | I_disjoint_2 t1 -> f0 t1
  | I_disjoint_3 (n1, t1', n2, t2', b, _d0) ->
    f1 n1 t1' n2 t2' b __ _d0
      (_disjoint_ind_rect f f0 f1 f2 f3 t1' (n2 :: t2') b _d0)
  | I_disjoint_4 (n1, t1', n2, t2', t0, _d0) ->
    f2 n1 t1' n2 t2' t0 __ _d0
      (_disjoint_ind_rect f f0 f1 f2 f3 (n1 :: t1') t2' t0 _d0)
  | I_disjoint_5 (n1, t1', n2, t2') -> f3 n1 t1' n2 t2' __ __

  (** val _disjoint_ind_rec :
      (bytes -> 'a1) -> (bytes -> 'a1) -> (interval -> bytes -> interval ->
      interval list -> bool -> __ -> _disjoint_ind -> 'a1 -> 'a1) ->
      (interval -> interval list -> interval -> bytes -> bool -> __ ->
      _disjoint_ind -> 'a1 -> 'a1) -> (interval -> interval list -> interval
  ...
  let rec _disjoint_ind_rec f f0 f1 f2 f3 _ _ _ = function
  | I_disjoint_1 t2 -> f t2
  | I_disjoint_2 t1 -> f0 t1
  | I_disjoint_3 (n1, t1', n2, t2', b, _d0) ->
    f1 n1 t1' n2 t2' b __ _d0
      (_disjoint_ind_rec f f0 f1 f2 f3 t1' (n2 :: t2') b _d0)
  | I_disjoint_4 (n1, t1', n2, t2', t0, _d0) ->
    f2 n1 t1' n2 t2' t0 __ _d0
      (_disjoint_ind_rec f f0 f1 f2 f3 (n1 :: t1') t2' t0 _d0)
  | I_disjoint_5 (n1, t1', n2, t2') -> f3 n1 t1' n2 t2' __ __

  (** val _disjoint_eq : bytes -> bytes -> _disjoint_ind **)

  let rec _disjoint_eq t1 t2 =
    let _evar_0_ = fun _ -> I_disjoint_1 t2 in
    let _evar_0_0 = fun n1 t1' ->
      let _evar_0_0 = fun _ -> I_disjoint_2 (n1 :: t1') in
      let _evar_0_1 = fun n2 t2' ->
        if Z.leb n1.imax n2.imin
        then I_disjoint_3 (n1, t1', n2, t2', (_disjoint t1' (n2 :: t2')),
               (_disjoint_eq t1' (n2 :: t2')))
        else if Z.leb n2.imax n1.imin
             then I_disjoint_4 (n1, t1', n2, t2',
                    (_disjoint (n1 :: t1') t2'),
                    (_disjoint_eq (n1 :: t1') t2'))
             else I_disjoint_5 (n1, t1', n2, t2')
      in
      (match t2 with
       | [] -> _evar_0_0
       | a :: l -> (fun _ -> _evar_0_1 a l))
  ...
  let disjoint t1 t2 =
    _disjoint (tobytes t1) (tobytes t2)
  ...
  let disjointP t1 t2 =
    ...
    let _evar_0_3 = fun _ _ _ _ -> ReflectF in
    let rec f _ _ _ = function
    | I_disjoint_1 t3 -> _evar_0_ t3
    | I_disjoint_2 t3 -> _evar_0_0 t3
    | I_disjoint_3 (n1, t1', n2, t2', b, _d0) ->
      _evar_0_1 n1 t1' n2 t2' b _d0 (fun _ _ -> f t1' (n2 :: t2') b _d0)
    | I_disjoint_4 (n1, t1', n2, t2', t0, _d0) ->
      _evar_0_2 n1 t1' n2 t2' t0 _d0 (fun _ _ -> f (n1 :: t1') t2' t0 _d0)
    | I_disjoint_5 (n1, t1', n2, t2') -> _evar_0_3 n1 t1' n2 t2'
    in f (tobytes t1) (tobytes t2) (_disjoint (tobytes t1) (tobytes t2))
         (_disjoint_eq (tobytes t1) (tobytes t2))
    ...
    | a :: l ->
      (match t2 with
       ...
       | a0 :: l0 ->
         if Z.leb a.imax a0.imin
         then _inter l (a0 :: l0)
         else if Z.leb a0.imax a.imin
              then _inter (a :: l) l0
              else let n = I.inter a a0 in
                   let n1' = { imin = a0.imax; imax = a.imax } in
                   let n2' = { imin = a.imax; imax = a0.imax } in
