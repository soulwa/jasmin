  ...
  Obj.magic coq_SubEqMixin coq_Z_eqType (fun x ->
    (&&)
      (Order.Order.le ring_display
        ...
        (GRing.zero (Num.NumDomain.porder_zmodType coq_Z_numDomainType)) x)
      (Order.Order.lt ring_display coq_Z_porderType x
        (Obj.magic modulus nbits))) (word_subType nbits)
